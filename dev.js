...
app.listen(port, () => {
    fs.readFile('data.txt', 'utf8' , (err, data) => {
        ...
        const setMinPrice = function (ctx) {
            ctx.telegram.sendMessage(ctx.update.callback_query.from.id, `\n\nТеперь давай выберем максимальная цена авто`, {
                reply_markup: {
                    inline_keyboard: [
                        [
        ...
        //
        //         if (notesIsChanged) {
        //             fs.writeFile("237016450.txt", JSON.stringify(notes), function(err) {
        //                 if (err) {
        //                     console.log(err);
        //                 }
        //             });
        ...
        //     }
        //
        //     fs.readFile('237016450.txt', 'utf8' , (err, data) => {
        //         if (err) {
        //             console.error(err)
        //             return
        //         }
        ...
        bot.hears('z', (ctx) => {
            ctx.telegram.sendMessage(ctx.message.chat.id, `Hello ${ctx.message.chat.id}`, {
                reply_markup: {
                    inline_keyboard: [[{text: '2000', callback_data: 'car'}, {text: '2001', callback_data: 'https//ya.ru'}]]
                }
            })
        })
